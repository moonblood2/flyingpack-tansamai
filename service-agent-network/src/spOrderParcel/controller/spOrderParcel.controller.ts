...
class SpOrderParcelController {
  ...
  async makeOrder(req: express.Request, res: express.Response) {
    ...
    const { orders, results, err } = await spOrderParcelService.makeOrder(
      token,
      req.body.origin,
      req.body.orders
    );
  ...
  async receiveOrderWeight(req: express.Request, res: express.Response) {
    ...
    const { err } = await orderService.updateSpOrderParcelIdAndTrackingCode(
      <string>resource.anOrderId,
      resource.spOrderParcelId,
      <string>resource.trackingCode
    );
    if (err.code === code.SUCCESS) {
      //Get url_webhook_order by user_id. Use getHookUrlOrder from userApi service.
      const { url, err } = await userApiService.getHookUrlOrder(
        <string>resource.userId
      );
      if (err.code === code.SUCCESS) {
        //Get reference_no by sp_order_parcel_id
        const { referenceNo, err } =
          await orderService.getReferenceNoByAnOrderId(
            <string>resource.anOrderId
          );
        if (err.code === code.SUCCESS) {
          const { orderProducts, err } =
            await orderService.getOrderProductsByAnOrderId(
              <string>resource.anOrderId,
              <string>resource.userId
            );
          if (err.code === code.SUCCESS) {
            //Map orderProducts to orderProductSerialNumbers, (key=productCode, value=anOrderProductId)
            let mapAnOrderProductId: Map<string, string> = new Map<
              string,
              string
            >();
                ...
                for (const s of item.serialNumbers) {
                  orderProductSerialNumbers.push({
                    anOrderProductId: <string>(
                      mapAnOrderProductId.get(<string>item.productCode)
                    ),
                    serialNumberStart: s,
                  });
                }
            ...
            const { err } = await serialNumberService.updateSerialNumbers(
              orderProductSerialNumbers
            );
  ...
  async receiveOrderStatus(req: express.Request, res: express.Response) {
    ...
    //Get reference_no by sp_order_parcel_id
    let { order, err } = await orderService.getOrderBySpOrderParcelId(
      <string>resource.spOrderParcelId
    );
    if (err.code === code.SUCCESS) {
      ...
      if (err.code === code.SUCCESS) {
        ...
        const { url, err } =
          await userApiService.getHookUrlOrderBySpOrderParcelId(
            <string>resource.spOrderParcelId
          );
        ...
        if (order.shippingStatus) {
          ...
          order.codStatus &&
          order.codStatus !== anOrderStatusMap.TRANSFERRED
        ) {
          // Ignore transferred status
